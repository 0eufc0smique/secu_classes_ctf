...
// on met la meta_heap a 12288 (multiple de 4096) pour eviter de la remap, on pourra donc faire 250+ chunks de meta
...
// fonction d'initialisation de log
void initialize_log() {
  // si la fonction log est initialisee alors on ne l'init pas
  if (log_file_descriptor != -1) {
    return;
  }

  // getenv msm recup le nom du fichier de log
  const char *log_filename = getenv("MSM_OUTPUT");
  // si log_filename n'est pas present, alors l'init a fail et on return  
  if (!log_filename) {
    return;
  }

  // man 2 open
  // ouverture en mode écriture (WRite ONLY), création du chemin du fichier si il n'existe pas (O_CREAT), ouverture en mode append
